...
function updateUI(state) {
    try {
        ...
        const noAircraftText = (I18N[currentLang] || I18N.fr)['telemetry.no_aircraft'];
        setText('t-aircraft', state.active_aircraft || noAircraftText);
...
// Verify critical telemetry DOM elements exist at load time
function verifyTelemetryDOM() {
    const requiredIds = [
        't-heading', 't-altitude', 't-airspeed', 't-bank',
        't-raw', 't-smoothed', 't-output', 't-coord', 't-damper',
        't-profile', 't-aircraft',
        'tv-rpm', 'tv-throttle', 'tv-fpm', 'tv-aoa',
        'tv-yaw_rate', 'tv-flap_vfe',
        't-hdr-heading', 't-compass-rose',
        't-gear-nose', 'tv-gear-nose', 't-gear-left', 'tv-gear-left',
        't-gear-right', 'tv-gear-right', 't-gear-strip'
    ];
    const missing = requiredIds.filter(id => document.getElementById(id) === null);
    if (missing.length > 0) {
        console.warn('[Telemetry] Missing DOM elements:', missing.join(', '));
    } else {
        console.log('[Telemetry] All', requiredIds.length, 'DOM elements verified.');
    }
}
...
// Connection status
// ---------------------------------------------------------------------------
function setConnected(connected, xplaneActive, xplaneState) {
    isConnected = connected;
    const dict = I18N[currentLang] || I18N.fr;
    const isLoading = xplaneState === 'loading';
    connDot.classList.toggle('connected', connected && xplaneActive);
    connDot.classList.toggle('loading', connected && isLoading);
    ...
    if (!connected) {
        ...
        badgeText = dict['status.loading'];
        badgeClass = 'status-badge loading';
        connTextStr = dict['status.loading_xplane'];
    } else {
    ...
    if (enterBtn) {
        ...
        enterBtn.classList.toggle('inactive-sim', !xplaneActive && !isLoading);
    }
    const launchBtn = document.getElementById('btn-launch-xplane');
    const launchText = document.getElementById('launch-xplane-text');
    if (launchBtn) {
        // Clear launchPending once X-Plane is detected as loading or ready
        if (launchPending && (xplaneActive || isLoading)) {
            launchPending = false;
        }
        const shouldShow = connected && !xplaneActive && !isLoading && !launchPending;
        launchBtn.style.display = shouldShow ? 'block' : 'none';
        if (shouldShow) {
            launchBtn.disabled = false;
            if (launchText) {
                launchText.textContent = dict['connect.launch_xplane'];
            }
        }
    }
}

let launchPending = false;  // true after clicking Launch, until state becomes ready/loading

async function launchXPlane() {
    const dict = I18N[currentLang] || I18N.fr;
    const btn = document.getElementById('btn-launch-xplane');
    const textEl = document.getElementById('launch-xplane-text');
    if (!btn || !textEl) return;
    btn.disabled = true;
    textEl.textContent = dict['connect.launching'];
    launchPending = true;  // block setConnected from showing the button
    try {
        const resp = await fetch('/api/launch-xplane', { method: 'POST', headers: {'Content-Type': 'application/json'} });
        const data = await resp.json();
        if (data.status !== 'ok') {
            textEl.textContent = data.message || dict['connect.launch_error'];
            btn.disabled = false;
            launchPending = false;
            setTimeout(() => { textEl.textContent = dict['connect.launch_xplane']; }, 3000);
        }
        // On success: launchPending stays true until setConnected() clears it
    } catch (e) {
        textEl.textContent = dict['connect.launch_error'];
        btn.disabled = false;
        launchPending = false;
        setTimeout(() => { textEl.textContent = dict['connect.launch_xplane']; }, 3000);
    }
