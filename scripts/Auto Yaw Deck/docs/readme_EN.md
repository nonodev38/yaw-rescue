...
└── Auto Yaw Deck/
    ├── panel.py               🖥️  GUI control panel (tkinter) — entry point
    ├── server.py              🐍 HTTPS server (Python) — entry point
    ├── start_panel.bat        🪟 Windows launcher
    │
    ├── deck/                  📦 Python package (server + panel logic)
    │   ├── __init__.py        🔗 Public re-exports (import compatibility)
    │   ├── paths.py           🗂️  Shared paths (static/, data/, certs/…)
    │   ├── log.py             📝 dlog() + debug flag (--debug / YAW_RESCUE_DEBUG=1)
    │   ├── prefs.py           💾 load/save prefs.json
    │   ├── state.py           📊 X-Plane state, cache, commands
    │   ├── cert.py            🔐 Self-signed SSL cert + networking
    │   ├── http.py            🌐 DeckHTTPHandler + API routes
    │   ├── xplane.py          🛩️  X-Plane detection
    │   ├── installer.py       📥 FlyWithLua / Auto Yaw Deck installer
    │   ├── flight.py          🛫 Resume at last airport
    │   ├── instance.py        🔄 PID + single-instance guard
    │   ├── panel_i18n.py      🌍 Panel FR/EN translations
    │   └── runner.py          🧵 Panel server thread
    │
    ├── static/                🖥️  Web resources
    │   ├── index.html         🌐 Web interface
    │   ├── css/               🎨 Dark aviation theme (5 sheets: theme,
    ...
    └── docs/                  📄 Documentation
        ├── readme_FR.md       📖 This guide (French)
...
| `--debug` | Debug logging `[debug]` (or env `YAW_RESCUE_DEBUG=1`) | off |
