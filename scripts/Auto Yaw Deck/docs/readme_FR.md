...
└── Auto Yaw Deck/
    ├── panel.py               🖥️  Panneau de contrôle graphique (tkinter) — entry point
    ├── server.py              🐍 Serveur HTTPS (Python) — entry point
    ├── start_panel.bat        🪟 Lanceur Windows
    │
    ├── deck/                  📦 Package Python (logique serveur + panneau)
    │   ├── __init__.py        🔗 Ré-exports publics (compat. imports)
    │   ├── paths.py           🗂️  Chemins partagés (static/, data/, certs/…)
    │   ├── log.py             📝 dlog() + flag de debug (--debug / YAW_RESCUE_DEBUG=1)
    │   ├── prefs.py           💾 load/save prefs.json
    │   ├── state.py           📊 État X-Plane, cache, commandes
    │   ├── cert.py            🔐 Certificat SSL auto-signé + réseau
    │   ├── http.py            🌐 DeckHTTPHandler + routes/API
    │   ├── xplane.py          🛩️  Détection X-Plane
    │   ├── installer.py       📥 Installation FlyWithLua / Auto Yaw Deck
    │   ├── flight.py          🛫 Reprise au dernier aéroport
    │   ├── instance.py        🔄 PID + anti-instances multiples
    │   ├── panel_i18n.py      🌍 Traductions FR/EN du panneau
    │   └── runner.py          🧵 Thread serveur du panneau
    │
    ├── static/                🖥️  Ressources web
    │   ├── index.html         🌐 Interface web
    │   ├── css/               🎨 Thème aviation sombre (5 feuilles : theme,
    │   │                          layout, widgets, responsive, modal)
    │   └── js/                ⚙️  Scripts (config, i18n, state, api, sliders,
    │                              ui, compass, vitals, actions, modal,
    │                              info, readme, main)
    │
    ...
    └── docs/                  📄 Documentation
        ├── readme_FR.md       📖 Ce guide (français)
        ├── readme_EN.md       📖 Ce guide (anglais)
...
| `--debug` | Logs de debug `[debug]` (ou env `YAW_RESCUE_DEBUG=1`) | désactivé |
