...
-- FIX: Use dataref() for string datarefs (dataref_table does not support strings)
dataref("dr_aircraft_str", "sim/aircraft/view/acf_descrip")
dataref("dr_acf_file_str", "sim/aircraft/view/acf_relative_path")

-- FIX: Use new dataref name (sim/cockpit2/controls/flap_ratio was replaced)
local dr_flap_ratio = dataref_table("sim/cockpit2/controls/flap_handle_request_ratio", "writable")
...
-- Aircraft detection — multiple datarefs for reliability
local function get_aircraft_name()
    local desc = dr_aircraft_str
    if desc and desc ~= "" and desc ~= "Cessna 172SP" then
        return desc
    end
    local acf_path = dr_acf_file_str
    if acf_path and acf_path ~= "" then
        local filename = acf_path:match("([^/\\]+)%.acf$")
        if filename and filename ~= "" then
            filename = filename:gsub("_ac$", "")
            return filename
        end
    end
    return desc or ""
...
local function detect_aircraft()
    local name = normalize_name(get_aircraft_name())
    if name == "" then
        return
    end
...
function toggle_auto_yaw_panel()
    if auto_yaw_wnd then
        ...
        float_wnd_set_title(auto_yaw_wnd, "YAW RESCUE")
...
create_command("plugin/auto_yaw/toggle_panel", "Yaw Rescue: Toggle panel", "toggle_auto_yaw_panel()", "", "")
create_command("plugin/auto_yaw/toggle_plugin", "Yaw Rescue: Toggle plugin", "toggle_auto_yaw()", "", "")
create_command("plugin/auto_yaw/reset", "Yaw Rescue: Reset", "reset_auto_yaw()", "", "")
add_macro("Yaw Rescue: ouvrir/fermer le panneau", "toggle_auto_yaw_panel()")
